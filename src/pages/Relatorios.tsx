...
import {
  ...
  FileText, Building2, RefreshCw, Printer, Truck, ShieldCheck, ShieldAlert,
...
function RelatorioExecutivo({filters,obras}:{filters:Filters;obras:Array<{id:string;nome:string}>}){
  ...
  const fetchData=useCallback(async()=>{if(!obraIds.length)return;setLoading(true);
    const [emp,veh,cron,cost,nc,dev,fvs,mov,maint]=await Promise.all([
      ...
      (supabase as any).from("v_almoxarifado_custos_obra").select("valor").in("obra_id",obraIds).gte("data_lancamento",filters.dataInicio).lte("data_lancamento",filters.dataFim),
    ...
    setStats({efetivo:new Set((emp.data??[]).map((e:any)=>e.employee_id)).size,veiculos:vehicleIds.size,progresso:Math.round(progress),previsto:(cost.data??[]).reduce((s:any,r:any)=>s+Number(r.valor_previsto??0),0),realizado:(cost.data??[]).reduce((s:any,r:any)=>s+Number(r.valor_realizado??0),0),ncs:nc.count??0,desvios:dev.count??0,inspecoes:fvs.count??0,consumo:(mov.data??[]).reduce((s:any,r:any)=>s+Number(r.valor??0),0),manutencoes:(maint.data??[]).filter((m:any)=>vehicleIds.has(m.vehicle_id)).length,atrasos:cronRows.filter((r:any)=>r.status_item==="atrasado").length});setFetched(true);setLoading(false);
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-base font-bold">Resumo executivo — {title}</h2><p className="text-xs text-muted-foreground">Consolidado de {fmtDate(filters.dataInicio)} até {fmtDate(filters.dataFim)}</p></div><Button size="sm" onClick={fetchData} disabled={loading}>{loading?<RefreshCw className="mr-2 h-4 w-4 animate-spin"/>:<TrendingUp className="mr-2 h-4 w-4"/>}{fetched?"Atualizar consolidado":"Gerar consolidado"}</Button></div>
    {!fetched?<EmptyPrompt icon={LayoutDashboard} label='Clique em "Gerar consolidado" para carregar a visão executiva da obra'/>:<>
      ...
      <Card><CardHeader><CardTitle className="text-sm">Pendências operacionais</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3">{[[Clock3,"Etapas atrasadas",stats.atrasos],[ShieldCheck,"NCs abertas",stats.ncs],[ClipboardCheck,"Inspeções reprovadas",stats.inspecoes],[ShieldAlert,"Desvios de segurança",stats.desvios],[Wrench,"Manutenções agendadas",stats.manutencoes],[Package,"Custo do almoxarifado",fmt(stats.consumo)]].map(([Icon,label,value]:any)=><div key={String(label)} className="flex items-center gap-3 rounded-lg border p-3"><Icon className="h-4 w-4 text-muted-foreground"/><div><p className="text-sm font-bold">{String(value)}</p><p className="text-[10px] text-muted-foreground">{String(label)}</p></div></div>)}</CardContent></Card></div>
    </>}</div>;
}
