...
function RelatorioExecutivo({filters,obras}:{filters:Filters;obras:Array<{id:string;nome:string}>}){
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-base font-bold">Resumo executivo — {title}</h2><p className="text-xs text-muted-foreground">Consolidado de {fmtDate(filters.dataInicio)} até {fmtDate(filters.dataFim)}</p></div><Button size="sm" onClick={fetchData} disabled={loading}>{loading?<RefreshCw className="mr-2 h-4 w-4 animate-spin"/>:<TrendingUp className="mr-2 h-4 w-4"/>}{fetched?"Atualizar consolidado":"Gerar consolidado"}</Button></div>
    {!fetched?<EmptyPrompt icon={LayoutDashboard} label='Clique em "Gerar consolidado" para carregar a visão executiva da obra'/>:<>
      ...
      <Card><CardHeader><CardTitle className="text-sm">Pendências operacionais</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3">{[[Clock3,"Etapas atrasadas",stats.atrasos],[ShieldCheck,"NCs abertas",stats.ncs],[ClipboardCheck,"Inspeções reprovadas",stats.inspecoes],[ShieldAlert,"Desvios de segurança",stats.desvios],[Wrench,"Manutenções agendadas",stats.manutencoes],[Package,"Consumo de materiais",fmt(stats.consumo)]].map(([Icon,label,value]:any)=><div key={String(label)} className="flex items-center gap-3 rounded-lg border p-3"><Icon className="h-4 w-4 text-muted-foreground"/><div><p className="text-sm font-bold">{String(value)}</p><p className="text-[10px] text-muted-foreground">{String(label)}</p></div></div>)}</CardContent></Card></div>
    </>}</div>;
}
