...
type Stock = { material_id: string; quantidade: number; materiais_catalogo: { nome: string; unidade: string; tipo_item: "consumo" | "retornavel" } | null };
  ...
  const load = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const [peopleResult, stockResult, historyResult] = await Promise.all([
        ...
        (supabase as any).from("almoxarifado_estoque").select("material_id,quantidade,materiais_catalogo(nome,unidade,tipo_item)").eq("obra_id", selected.id).gt("quantidade", 0),
