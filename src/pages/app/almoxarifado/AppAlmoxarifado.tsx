...
type Stock = { material_id: string; quantidade: number; materiais_catalogo: { nome: string; unidade: string } | null };
...
export default function AppAlmoxarifado() {
  ...
  const [history, setHistory] = useState<Delivery[]>([]); const [tab, setTab] = useState<"delivery" | "history">("delivery");
  ...
  const load = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const [peopleResult, stockResult, historyResult] = await Promise.all([
        (supabase as any).from("obra_funcionarios").select("employees(id,nome,cargos(nome))").eq("obra_id", selected.id).eq("status", true),
        (supabase as any).from("almoxarifado_estoque").select("material_id,quantidade,materiais_catalogo(nome,unidade)").eq("obra_id", selected.id).gt("quantidade", 0),
      ...
      setEmployees((peopleResult.data ?? []).map((row: any) => ({ id: row.employees.id, nome: row.employees.nome, cargo: row.employees.cargos?.nome })));
  ...
  return <main className="almox-shell"><header><div><span className="eyebrow">OBRA VINCULADA</span><h1>{obra?.nome ?? "Acesso bloqueado"}</h1><p>{operator?.nome}</p></div><button className="logout" onClick={()=>supabase.auth.signOut()}>Sair</button></header>
    {!obra ? <section className="blocked"><b>Acesso indisponível</b><p>{message}</p></section> : <><nav><button className={tab==="delivery"?"active":""} onClick={()=>setTab("delivery")}>Nova saída</button><button className={tab==="history"?"active":""} onClick={()=>setTab("history")}>Histórico</button></nav>{message && <div className={message.includes("sucesso")?"success":"alert"}>{message}</div>}
    ...
    : <section className="card history"><h2>Últimas entregas</h2>{history.length===0?<p>Nenhuma entrega registrada.</p>:history.map(row=><article key={row.id}><div><b>Comprovante #{row.numero}</b><span>{row.employees?.nome} · {row.frente}</span></div><time>{new Date(row.created_at).toLocaleString("pt-BR")}</time></article>)}</section>}</>}</main>;
}
