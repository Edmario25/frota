...
export function GovernancaAba({onCreateReview}:{onCreateReview:()=>void}){
  ...
  const {data,isLoading,isError,error,refetch}=useQuery({queryKey:["access-governance"],queryFn:async()=>{
  ...
  if(isLoading)return <p className="py-10 text-center text-muted-foreground">Carregando revisões de acesso...</p>;
  if(isError)return <Card><CardContent className="flex flex-col items-center gap-3 p-8 text-center"><ShieldAlert className="h-8 w-8 text-destructive"/><div><p className="font-medium">Não foi possível carregar as revisões de acesso.</p><p className="mt-1 text-sm text-muted-foreground">{error instanceof Error?error.message:"Atualize o banco e tente novamente."}</p></div><Button variant="outline" onClick={()=>refetch()}>Tentar novamente</Button></CardContent></Card>;
  return <div className="space-y-4">
    <div className="grid gap-3 md:grid-cols-3"><Card><CardContent className="flex gap-3 p-4"><UserCheck className="text-emerald-600"/><div><p className="font-semibold">Acessos monitorados</p><p className="text-xs text-muted-foreground">Contas, vínculos, validade e perfis são verificados automaticamente.</p></div></CardContent></Card><Card><CardContent className="flex gap-3 p-4"><CalendarClock className="text-primary"/><div><p className="font-semibold">Revisão periódica</p><p className="text-xs text-muted-foreground">Confirme quem mantém, ajusta ou perde acesso.</p></div></CardContent></Card><Card><CardContent className="flex gap-3 p-4"><History className="text-primary"/><div><p className="font-semibold">Histórico preservado</p><p className="text-xs text-muted-foreground">Decisões e revogações ficam rastreáveis.</p></div></CardContent></Card></div>
    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{(data?.alertas??[]).map(a=><Card key={a.tipo} className={a.severidade==="alta"?"border-red-300":a.severidade==="media"?"border-amber-300":""}><CardContent className="flex gap-3 p-4"><ShieldAlert className={a.severidade==="alta"?"text-red-600":a.severidade==="media"?"text-amber-600":"text-blue-600"}/><div><b className="text-2xl">{a.quantidade}</b><p className="text-sm">{a.descricao}</p><Badge variant="outline" className="mt-2 capitalize">Prioridade {a.severidade}</Badge></div></CardContent></Card>)}{(data?.alertas?.length??0)===0&&<Card className="md:col-span-2 lg:col-span-3"><CardContent className="flex items-center justify-center gap-2 p-6 text-emerald-700"><CheckCircle2/>Nenhuma pendência de governança identificada.</CardContent></Card>}</div>
